...
# from time import perf_counter
from params import (  # , CHEMIN_CACHE, CHEMIN_CYCLA
    D_MAX_POUR_NŒUD_LE_PLUS_PROCHE,
    LOG_PB,
)
from petites_fonctions import LOG, distance_euc
...
class Graphe_nx:
    ...
    def geom_arête(self, s, t):
        ...
        arête = min(
            (a for _, a in self.multidigraphe[s][t].items()), key=lambda a: a["length"]
        )
        if "name" in arête:
            nom = arête["name"]
        else:
        ...
        for s in self.multidigraphe.nodes:
            for t in self.multidigraphe[s]:
                for i in self.multidigraphe[s][t]:
                    à_supprimer = [
                        att
                        for att in self.multidigraphe[s][t][i]
                        if att not in à_garder
                    ]
        ...
        for a in self.multidigraphe[s][t].values():
            if "name" in a:
                ...
                if isinstance(a["name"], str):
                    ...
                    res.extend(
                        (r, piétonne and "place" in r.lower()) for r in a["name"]
                    )
        return res
    ...
    def villes_dun_sommet(self, s, bavard=0):
        ...
        try:
            ...
            if bavard > 0:
                print(
                    f"Pas de ville en mémoire pour le sommet {s}.  Voici ses données\n {self.digraphe[s]}."
                )
            return []
    ...
    def voisins(self, s, p_détour, interdites={}):
        ...
        def cycla_corrigée(voisin):
            return self.cyclabilité.get((s, voisin), 1.0) ** (p_détour * 1.5)

        if s in interdites:
            return (
                (voisin, données["length"] / cycla_corrigée(voisin))
                for (voisin, données) in self.digraphe[s].items()
                if voisin not in interdites[s]
            )
        else:
            return (
                (voisin, données["length"] / cycla_corrigée(voisin))
